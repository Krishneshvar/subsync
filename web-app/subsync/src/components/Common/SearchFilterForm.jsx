...
const SelectInput = ({ value, onChange, options, placeholder, label }) => (
  <div className="w-full md:w-auto">
    <Select value={value} onValueChange={onChange}>
      ...
      <SelectContent>
        {options.map(opt => <SelectItem key={opt.key} value={opt.label}>{opt.label}</SelectItem>)}
      </SelectContent>
    </Select>
  </div>
)
...
export default function SearchFilterForm({
  ...
  return (
    <div className="w-full md:w-auto flex flex-wrap items-center gap-2 mb-6">
      ...
      <SelectInput label="Order" value={order} onChange={setOrder} options={[{ label: 'Ascending' }, { label: 'Descending' }]} placeholder="Order" />
    </div>
  )
}
